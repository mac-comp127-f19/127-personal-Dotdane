...
//            .isBlank();
        ...
        String upperCaseNoAText = upperCaseText.replace("A", "");
        String upperCaseNoANoExclamationText = upperCaseNoAText.replace("!", "");
        return upperCaseNoANoExclamationText.isBlank();
    }
...
//            String.valueOf(
//                text.length()));
        boolean trueOrFalseResult;
        String textLengthString = "" + (float) text.length();
        int textLengthDoubled = (int)Float.parseFloat(textLengthString) + (int)Float.parseFloat(textLengthString);
        if (text.contains("" + (textLengthDoubled / 2))) {
            trueOrFalseResult = text.contains("" + text.length());
        } else {
            return false;
        }
