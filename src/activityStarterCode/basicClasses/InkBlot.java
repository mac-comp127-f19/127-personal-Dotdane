...
public class InkBlot {
    ...
    public void generateInkBlot() {
        ...
        RandomWalk x = new RandomWalk(canvas.getWidth() / 2);
        RandomWalk y = new RandomWalk(canvas.getHeight() / 2);
        RandomWalk r = new RandomWalk(0);
        RandomWalk g = new RandomWalk(0);
        RandomWalk b = new RandomWalk(0);
        int maxIterations = 15000;
        Line newLine;

        for (int i = 0; i < maxIterations; i++) {
            newLine = advance(x, y, 0, 300);
            newLine.setStrokeColor(getColor(r, g, b));
//            newLine.setStrokeColor(Color.magenta);
            canvas.add(newLine);
            canvas.add(mirror(newLine));
        }


//        Epicycle x = new Epicycle(canvas.getWidth() / 2, 1);
//        Epicycle y = new Epicycle(canvas.getHeight() / 2, 1);
//        RandomWalk r = new RandomWalk(0);
//        RandomWalk g = new RandomWalk(0);
//        RandomWalk b = new RandomWalk(0);
//        int maxIterations = 15000;
...
//            return Color.darkGray;
            red = new RandomWalk(150);
            green = new RandomWalk(150);
            blue = new RandomWalk(150);
            return getColor(red, green, blue);
        }
    }

    private Color getColor(Epicycle red, Epicycle green, Epicycle blue) {
        int r = red.advanceValue();
        int g = green.advanceValue();
        int b = blue.advanceValue();
        int rate = red.t;
...
//            return Color.darkGray;
            red = new Epicycle(150, rate);
            green = new Epicycle(150, rate);
            blue = new Epicycle(150, rate);
            return getColor(red, green, blue);
        }
    }
     ...
     */
    private Line advance(RandomWalk xWalk, RandomWalk yWalk, int min, int max) {
        int step = 1;
        int oldX = xWalk.getValue();
        int oldY = yWalk.getValue();
        int newX = xWalk.advanceValue(step);
        int newY = yWalk.advanceValue(step);
        while (newX < min || newX > max) {
            newX = xWalk.advanceValue(step);
        }
        while (newY < min || newY > max) {
            newY = yWalk.advanceValue(step);
        }
    ...
    private Line advance(Epicycle xWalk, Epicycle yWalk, int min, int max) {
        int step = 1;
        int oldX = xWalk.getValue();
        int oldY = yWalk.getValue();
        int newX = xWalk.advanceValue(step);
